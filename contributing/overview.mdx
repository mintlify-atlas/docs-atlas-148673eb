---
title: Contributing
description: How to contribute to ScaleTail
---
...
Follow these steps to add a new service to the ScaleTail collection:

<Steps>
  <Step title="Copy the service template">
    Copy `templates/service-template` into `services/<service-name>` and rename the compose and README files accordingly.
  </Step>

  <Step title="Update compose.yaml">
    Make the following changes to your `compose.yaml`:
  ...
  <Step title="Configure the proxy port">
    Update `"Proxy":"http://127.0.0.1:80"` in `compose.yaml` with the app's actual internal port.
    
    <Warning>
      The proxy configuration does not consume `.env` values automatically. You must specify the port directly.
    </Warning>
    
    Remove `TS_SERVE_CONFIG` if Serve/Funnel is not needed.
  </Step>

  <Step title="Fill in the service README">
  ...
  <Step title="Validate your configuration">
    Sanity-check the stack with `docker compose config` from the service directory to catch typos and missing variables:
...
All contributions are released under the MIT license. By submitting a pull request, you agree to license your contribution under the same terms.